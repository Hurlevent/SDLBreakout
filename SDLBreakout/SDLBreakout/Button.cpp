...
namespace Breakout {
		...
		int Button::listen_to_click(const Window * win, const InputManager * input)
		{
			int clicked = 0;
			SDL_Point mouse_pos;
		...
		void Button::render_object(const Window * win, const InputManager * input)
		{
			// std::cout << "Rendering Texture: " << m_texture_id << ", Rect: " << m_properties.x << ", " << m_properties.y << ", " << m_properties.w << ", " << m_properties.h << std::endl;
			win->render_texture(m_texture_id, &m_properties, nullptr);
		}
}